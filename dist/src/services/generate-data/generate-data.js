...
let generateProducts = async (strapi, count = 50) => {
    try {
        ...
        const imageUrls = new Array(count).fill(null).map(() => new Array(faker_1.faker.datatype.number({ min: 3, max: 6 })).fill(null)
        ...
        await Promise.all(new Array(count).fill(null).map((el, index) => {
            ...
            return axios_1.default.post('http://localhost:1337/api/upload', formData);
        }));
    }
    catch (err) {
